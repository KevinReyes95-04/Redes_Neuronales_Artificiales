...
# ─────────────────────────────────────────────────────────────────────────────
print("="*70)
print("PREGUNTA 5 – Comparación sistemática de arquitecturas")
print("="*70)
...
for (capas, neuronas, lr) in configs:
    ...
    resultados.append({
        ...
        "Acc Train (máx)":   round(train_acc, 4),
        "Acc Val (máx)":     round(val_acc,   4),
        "Acc Test":          round(acc_test,  4),
    })
    historiales[lbl] = hist
    print(f"  {lbl:30s} | Test Acc = {acc_test:.4f}")
...
# Mejor configuración
best_row = df_res.loc[df_res["Acc Test"].idxmax()]
best_cfg = best_row["Configuración"]
print(f"\n★ Mejor configuración (mayor Acc Test): {best_cfg}")
