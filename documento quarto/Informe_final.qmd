...
```{python}
#| echo: false
#| tbl-cap: "Estadísticos descriptivos por categoría de severidad."
import pandas as pd
from IPython.display import Markdown, display

def mostrar_tabla(tabla, decimales=4):
    """Muestra una tabla Markdown sin índice y adaptable a HTML y PDF."""
    tabla = tabla.copy()
    columnas_numericas = {
        columna
    ...
    def formatear(valor, columna):
        if pd.isna(valor):
            return "-"
        if columna in columnas_numericas:
            if isinstance(valor, (int,)) or (
    ...
    alineacion = (
        "| "
        + " | ".join(
            "---:" if columna in columnas_numericas else ":---"
            for columna in tabla.columns
        )
        + " |"
    )
    filas = [
        "| "
        + " | ".join(
            formatear(valor, columna)
            for columna, valor in zip(tabla.columns, fila)
        )
        + " |"
        for fila in tabla.itertuples(index=False, name=None)
    ]
    display(Markdown("\n".join([encabezado, alineacion, *filas])))

tabla_p1_descriptivos = pd.read_csv(
    "../resultados/tablas/04_estadisticos_descriptivos_por_severidad.csv"
).rename(
    columns={
        "severity": "Severidad",
        "descripcion": "Predictor",
        "media": "Media",
...
![Distribución general de las variables predictoras](../resultados/figuras/01_distribuciones_variables.png){#fig-distribuciones-variables width="75%"}
...
#| tbl-cap: "Resumen de calidad de los datos."
import pandas as pd
tabla_calidad = pd.read_csv(
    "../resultados/tablas/01_resumen_calidad_datos.csv"
).rename(columns={"indicador": "Indicador", "valor": "Valor"})
mostrar_tabla(tabla_calidad, decimales=0)
```
...
![Distribución de los predictores por categoría de severidad](../resultados/figuras/02_boxplots_por_severidad.png){#fig-boxplots-severidad width="75%"}
...
![Medias de las variables predictoras por categoría de severidad](../resultados/figuras/03_medias_por_severidad.png){#fig-medias-severidad width="75%"}
...
```{python}
#| echo: false
#| label: tbl-spearman-severidad
#| tbl-cap: "Correlación de Spearman entre cada predictor y la severidad."
import pandas as pd
tabla_p1_correlaciones = pd.read_csv(
    "../resultados/tablas/05_correlacion_spearman_con_severidad.csv"
).rename(
    columns={
        "descripcion": "Predictor",
        "rho_spearman": "Rho de Spearman",
        "valor_p": "p-valor",
    }
)
mostrar_tabla(
    tabla_p1_correlaciones[["Predictor", "Rho de Spearman", "p-valor"]],
    decimales=4,
)
```
...
: Balance del conjunto de entrenamiento antes y después de SMOTE.
...
tabla_p3 = tabla_p3[
    ...
    columns={
        ...
        "Accuracy entrenamiento": "Exactitud entrenamiento",
        "Accuracy validacion": "Exactitud validación",
        "Accuracy prueba": "Exactitud prueba",
    }
)
tabla_p3["Validación"] = tabla_p3["Validación"].replace(0, "-")
mostrar_tabla(tabla_p3.fillna("-"), decimales=4)
```
...
tabla_p4_folds = tabla_p4_folds[
    ...
    columns={
        ...
        "Loss validacion": "Pérdida validación",
    }
)
mostrar_tabla(tabla_p4_folds, decimales=4)
```
...
tabla_p4_comp = tabla_p4_comp.rename(
    columns={
        ...
        "Conjunto evaluado": "Conjunto",
        "Accuracy medio": "Exactitud",
        "Desviacion accuracy": "Desv. exact.",
        "F1 macro medio": "F1 macro",
        "Desviacion F1 macro": "Desv. F1 macro",
    }
)
tabla_p4_comp["Estrategia"] = tabla_p4_comp["Estrategia"].replace(
    {
        "Particion fija 70/15/15": "Partición 70/15/15",
        "K-fold estratificado (k=5)": "k-fold estratificado (k=5)",
    }
)
tabla_p4_comp["Conjunto"] = tabla_p4_comp["Conjunto"].replace(
    {
        "Prueba independiente": "Prueba independiente",
        "Validaciones internas": "Validaciones internas",
    }
)
mostrar_tabla(tabla_p4_comp.fillna("-"), decimales=4)
```
...
#| tbl-colwidths: [16, 8, 10, 9, 8, 17, 17, 15]
import pandas as pd
...
tabla_p5_resumen = tabla_p5[
    ...
    columns={
        ...
        "Tasa aprendizaje": "Tasa (lr)",
        "Épocas ejecutadas": "Épocas",
        "Loss Train mínima": "Pérd. entren.",
        "Loss Val mínima": "Pérd. valid.",
        "Acc Val (máx)": "Exact. valid.",
    }
)
mostrar_tabla(tabla_p5_resumen, decimales=4)
```
...
tabla_p6["Función de pérdida"] = tabla_p6["Función de pérdida"].replace(
    {
        "Entropia cruzada categorica": "Entropía cruzada categórica",
        "Error cuadratico medio": "Error cuadrático medio",
    }
)
tabla_p6["Decisión metodológica"] = tabla_p6["Decisión metodológica"].replace(
    {
        "Recomendada para clasificacion multiclase con softmax":
            "Recomendada para clasificación multiclase con softmax",
    }
)
mostrar_tabla(tabla_p6, decimales=4)
```
...
].rename(columns={"Accuracy": "Exactitud"})
mostrar_tabla(tabla_p7_resumen, decimales=4)
```
...
tabla_p7_metricas = tabla_p7_metricas.rename(columns={"Precision": "Precisión"})
mostrar_tabla(tabla_p7_metricas, decimales=4)
```
...
mostrar_tabla(
    pd.read_csv("../resultados/tablas/p8_distribucion_binaria.csv").rename(
...
tabla_p9_particion = pd.read_csv("../resultados/tablas/p9_particion_binaria.csv")
tabla_p9_particion["Conjunto"] = tabla_p9_particion["Conjunto"].replace(
    {"Validacion": "Validación"}
)
mostrar_tabla(
    tabla_p9_particion,
...
mostrar_tabla(
    pd.read_csv("../resultados/tablas/p9_metricas_binarias_umbral_05.csv").rename(
        columns={"Precision": "Precisión"}
    ),
    decimales=4,
)
```
...
mostrar_tabla(
    pd.read_csv(
        "../resultados/tablas/p9_comparacion_mlp_multiclase_binario.csv"
    ).rename(
        columns={
            "Accuracy": "Exactitud",
            "Precision macro": "Precisión macro",
            "Precision ponderada": "Precisión ponderada",
        }
    ),
    decimales=4,
)
...
#| tbl-colwidths: [18, 12, 12, 12, 12, 12, 10, 12]
import pandas as pd

tabla_p10_youden = pd.read_csv(
    "../resultados/tablas/p10_metricas_umbral_youden.csv"
)[
    [
        "Criterio umbral",
        "Umbral",
...
mostrar_tabla(tabla_p10_youden, decimales=4)
```
...
#| tbl-colwidths: [18, 12, 12, 14, 14, 12, 9, 9]
import pandas as pd

tabla_p10_umbrales = pd.read_csv(
    "../resultados/tablas/p10_comparacion_umbrales.csv"
)[
    [
        "Criterio",
        "Umbral",
    ...
    columns={
        ...
        "Sensibilidad": "Sens.",
        "Especificidad": "Espec.",
        "F1 Enfermo": "F1 enferm.",
    }
)
mostrar_tabla(tabla_p10_umbrales, decimales=4)
```
...
```{python}
#| echo: false
#| tbl-colwidths: [16, 14, 14, 12, 18, 12, 14]
import pandas as pd
...
tabla_p11_mostrar = tabla_p11_mostrar[
    ...
    columns={
        "Error Std": "Error est.",
        "Significativa (alpha=0.05)": "Signif.",
        "Signo esperado": "Signo esp.",
    }
)
for columna in ["Coeficiente", "Error est.", "p-valor"]:
    tabla_p11_mostrar[columna] = tabla_p11_mostrar[columna].round(4)
mostrar_tabla(tabla_p11_mostrar, decimales=4)
```
...
```{python}
#| echo: false
#| label: tbl-p11-intercepto
#| tbl-cap: "Intercepto estimado de la regresión logística."
mostrar_tabla(
    pd.read_csv("../resultados/tablas/p11_intercepto_logistico.csv").rename(
        columns={"Parametro": "Parámetro"}
    ),
    decimales=4,
)
```
...
```{python}
#| echo: false
#| label: tbl-p12-parametros-lasso
#| tbl-cap: "Parámetro seleccionado por la regresión logística Lasso."
import pandas as pd
mostrar_tabla(
    pd.read_csv("../resultados/tablas/p12_parametros_lasso.csv").rename(
        columns={
            "Metodo": "Método",
            "Validacion cruzada": "Validación cruzada",
            "C optimo": "C óptimo",
        }
    ),
    decimales=4,
...
```{python}
#| echo: false
#| label: tbl-p12-importancia-lasso
#| tbl-cap: "Importancia de variables según la magnitud de los coeficientes Lasso."
...
mostrar_tabla(tabla_p12_lasso, decimales=4)
```
...
```{python}
#| echo: false
#| label: tbl-p12-comparacion-importancia
#| tbl-cap: "Comparación entre la importancia de Lasso y la importancia por permutación del MLP."
tabla_p12_comp = pd.read_csv("../resultados/tablas/p12_comparacion_lasso_mlp_pfi.csv")
tabla_p12_comp = tabla_p12_comp[
    [
        "Variable",
        "Ranking Lasso",
        "Abs(Coef)",
...
tabla_p12_comp = tabla_p12_comp.rename(
    columns={
        "Abs(Coef)": "Magnitud Lasso",
        "Ranking MLP PFI": "Ranking MLP",
        "Importancia PFI": "Importancia MLP",
    }
)
mostrar_tabla(tabla_p12_comp, decimales=4)
```
...
#| tbl-colwidths: [19, 11, 12, 12, 12, 12, 10, 12]
import pandas as pd
tabla_p13_05 = pd.read_csv(
    "../resultados/tablas/p13_comparacion_modelos_umbral_05.csv"
)[
    [
        "Modelo",
        "Umbral",
...
mostrar_tabla(tabla_p13_05, decimales=4)
```
...
#| tbl-colwidths: [19, 11, 12, 12, 12, 12, 10, 12]
import pandas as pd
tabla_p13_youden = pd.read_csv(
    "../resultados/tablas/p13_comparacion_modelos_youden.csv"
)[
    [
        "Modelo",
        "Umbral",
...
mostrar_tabla(tabla_p13_youden, decimales=4)
```
...
mostrar_tabla(tabla_p14_pfi, decimales=4)
```
...
mostrar_tabla(tabla_p14_comp, decimales=4)
```
...
#| tbl-colwidths: [35, 65]
import pandas as pd
tabla_p15_resumen = pd.read_csv("../resultados/tablas/p15_resumen_grilla.csv")
tabla_p15_resumen["Indicador"] = tabla_p15_resumen["Indicador"].replace(
    {
        "Dimension de la grilla": "Dimensión de la grilla",
        "Metodo de asignacion": "Método de asignación",
    }
)
tabla_p15_resumen["Valor"] = tabla_p15_resumen["Valor"].replace(
    {
        "Muestra aleatoria sin reemplazo y ubicacion secuencial en la grilla":
            "Muestra aleatoria sin reemplazo y ubicación secuencial en la grilla",
    }
)
mostrar_tabla(tabla_p15_resumen, decimales=0)
```
...
import pandas as pd
tabla_p16_metodo = pd.read_csv(
    "../resultados/tablas/p16_metodo_moran_ordinal.csv"
).rename(columns={"Caracteristica": "Característica"})
tabla_p16_metodo["Característica"] = tabla_p16_metodo["Característica"].replace(
    {
        "Metodo elegido": "Método elegido",
        "Numero de celdas evaluadas": "Número de celdas evaluadas",
        "Normalizacion de pesos": "Normalización de pesos",
    }
)
tabla_p16_metodo["Valor"] = tabla_p16_metodo["Valor"].replace(
    {"Indice de Moran global": "Índice de Moran global"}
)
mostrar_tabla(tabla_p16_metodo, decimales=4)
```
...
tabla_p16_resultados = pd.read_csv(
    "../resultados/tablas/p16_moran_global_ordinal.csv"
).rename(columns={"Estadistico": "Estadístico"})
tabla_p16_resultados["Estadístico"] = tabla_p16_resultados["Estadístico"].replace(
    {
        "Indice de Moran global (I)": "Índice de Moran global (I)",
        "Estadistico Z": "Estadístico Z",
    }
)
mostrar_tabla(tabla_p16_resultados, decimales=4)
```
...
#| tbl-colwidths: [35, 65]
import pandas as pd
tabla_p17_config = pd.read_csv(
    "../resultados/tablas/p17_configuracion_mlp_coordenadas.csv"
).rename(columns={"Caracteristica": "Característica"})
tabla_p17_config["Característica"] = tabla_p17_config["Característica"].replace(
    {
        "Clasificacion usada": "Clasificación usada",
        "Umbral de clasificacion": "Umbral de clasificación",
    }
)
mostrar_tabla(tabla_p17_config, decimales=4)
```
...
#| tbl-colwidths: [27, 9, 14, 14, 18, 18]
mostrar_tabla(
    pd.read_csv("../resultados/tablas/p17_comparacion_mlp_coordenadas.csv").rename(
        columns={
            "Numero predictores": "Pred.",
            "Accuracy": "Exact.",
            "Delta Accuracy vs base": "Cambio exact.",
            "Delta AUC vs base": "Cambio AUC",
        }
    ),
    decimales=4,
)
...
#| tbl-colwidths: [20, 28, 12, 40]
import pandas as pd
mostrar_tabla(
    pd.read_csv(
        "../resultados/tablas/p18_configuracion_estrategias_espaciales.csv"
    ).rename(
        columns={
            "Numero predictores": "Pred.",
...
#| tbl-colwidths: [36, 14, 14, 18, 18]
mostrar_tabla(
    pd.read_csv(
        "../resultados/tablas/p18_comparacion_estrategias_espaciales.csv"
    ).rename(
        columns={
            "Accuracy": "Exact.",
            "Delta Accuracy vs base": "Cambio exact.",
...
import pandas as pd
mostrar_tabla(
    pd.read_csv("../resultados/tablas/p19_configuracion_propuesta.csv").rename(
        columns={
            "Extension": "Extensión",
            "Implementacion": "Implementación",
        }
...
tabla_p19_modelos = pd.read_csv(
    "../resultados/tablas/p19_comparacion_modelos.csv"
)[
    [
        "Modelo",
        "Umbral",
...
mostrar_tabla(tabla_p19_modelos, decimales=4)
```
...
#| tbl-colwidths: [22, 12, 18, 18, 18, 12]
mostrar_tabla(
    pd.read_csv("../resultados/tablas/p19_resumen_dropout.csv").rename(
        columns={
            "Epocas entrenadas": "Épocas",
            "Loss train final": "Pérd. entren.",
            "Loss val final": "Pérd. valid.",
            "Loss val minima": "Pérd. valid. mín.",
            "Epoca mejor val_loss": "Mejor época",
        }
    ),
    decimales=4,
)
...
mostrar_tabla(
    pd.read_csv("../resultados/tablas/p19_importancia_random_forest.csv"),
    decimales=4,
)
```
